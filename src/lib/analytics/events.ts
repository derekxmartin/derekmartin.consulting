...
type EventName = 'service_cta_click' | 'case_study_view' | 'demo_open' | 'form_start' | 'generate_lead' | 'form_error' | 'email_click' | 'phone_click' | 'linkedin_click';
...
const eventKeys: Record<EventName, (keyof SafeParams)[]> = {
  ...
  form_start: ['form_id'], generate_lead: ['form_id', 'service_id'], form_error: ['form_id', 'error_category'], email_click: ['placement'], phone_click: ['placement'], linkedin_click: ['placement'],
};
// The consent component supplies the adapter only after analytics is accepted.
