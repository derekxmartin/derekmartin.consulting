...
export function WorkCards({ limit }: { limit?: number }) {
  ...
  return <div className={styles.workGrid}>{cases.map(c => <article className={styles.workCard} key={c.slug}>
    ...
    <span className={styles.evidenceLabel}>{c.evidenceType}{c.inProgress && ' · In progress'}</span><h2><Link href={`/work/${c.slug}`}>{c.title}</Link></h2><p>{c.summary}</p><p>{c.platforms.join(' · ')}</p>
    <p><Link className="text-link" href={`/work/${c.slug}`}>{c.inProgress ? 'Read the draft case study' : 'Read the case study'} <Arrow /></Link></p>
    {c.demoUrl && <a className="text-link" href={c.demoUrl} target="_blank" rel="noopener noreferrer">{c.inProgress ? 'Explore the static preview' : 'Open live demo'} <Arrow /></a>}
  </article>)}</div>;
}
